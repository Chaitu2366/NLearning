...
public class DeltaCertificateExam_IT extends NLETestBase {
	...
	public void init() {
		try {
			sysId = nleEntitlementUtils.getExamPath("delta", "x_snc_lxp_m2m_course_path","Exam");
	...
	@Step(value = 0, info = "Signed In User - enroll a Delta-Certificate Path")
	...
	@Step(value = 2, info = "Verify Delta-certificate with Assigned state")
	...
	@Step(value = 16, info = "Verify Delta-Certificate Popup")
	...
	@Step(value = 18, info = "Verify Delta-Certificate is generated for the user ")
	...
	@Step(value = 20, info = "Verify Delta-Certificate badge notification generated for the user")
	public void VerifyMicroCertificateNotifcation() {
		String deltaBadgeSysid = leutils.getGlideRecordEncodedQuery("x_snc_lxp_certification", "badge","sys_id="+certSysid);
		System.out.println(nleEntitlementUtils.badgeCompletion
				.replace("badgetype", nleEntitlementUtils.getName("x_snc_lxp_badge", deltaBadgeSysid)));
		Assert.assertTrue("Badge Completion email is not generated",
				nleEntitlementUtils.verifyEmailVerification(impersonateUserEmail, nleEntitlementUtils.getName("x_snc_lxp_badge", deltaBadgeSysid)));
	}

	@Step(value = 22, info = "Verify Delta-Certificate on certification tab")
	public void VerifyMicroCertificateUI() {
		...
		Assert.assertEquals("Delta Certificate is generated", "Current", certificateStatus);

	}

	@Step(value = 24, info = "Verify Delta-Certificate on transcript tab")
